...
const ProductCard = ({
  ...
  return (
    <Link to={`/product/${id}`} className="block">
      <div 
        className="bg-white rounded-lg shadow-md p-4 w-64 font-sans transition-all duration-200 hover:shadow-lg relative cursor-pointer"
        ...
        {farmName && (
          <p className="text-gray-500 text-sm mb-2 text-left">
            ...
            {vendorId ? (
              <Link
                to={`/vendors/${vendorId}`}
                className="text-market-green hover:text-market-green-dark transition-colors underline"
